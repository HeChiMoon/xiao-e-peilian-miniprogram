...
- 训练推荐和知识推荐已上线基础版，能够结合老人资料、最近测评、最近姿势检测和今日训练进度进行规则化推荐
- 小鹅图标体系已接入核心页面，当前图标资源包括 14 个功能图标和 4 个状态表情
- 测评页已优化五次坐站、扶椅单腿站立的计时按钮高光，报告页已按维度重要程度做排序和颜色标注
- 健康知识页已加入本地点赞、收藏和留言交互，用于 Demo 展示，不作为云端社交系统
...
身份选择 -> 注册建档 -> 首页 -> 测评 / 每日练 / 健康知识 / 小百科 -> 我的
