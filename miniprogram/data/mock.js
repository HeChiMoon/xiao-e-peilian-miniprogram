const activeTrainingLevels = [
  {
    ...
    image: '/assets/images/penguin/encourage.jpg',
  ...
  },
  {
    ...
    image: '/assets/images/penguin/worried.jpg',
...
const assessmentSections = {
  intro: {
    title: '小鹅腿脚测评',
    greeting: '爷爷奶奶好！欢迎来到小鹅腿脚测评。咱们今天像做小检查一样，看看腿脚灵不灵便。',
    safety: '旁边一定要有一把稳固的椅子或墙壁扶着，最好有家人在旁边看着。如果觉得疼或者站不稳，马上停下来，不要勉强！'
  },
  basicQuestions: [
    {
      key: 'walking',
      title: '平时走路怎么样？',
      options: [
        { label: '完全自己走', value: 'independent', score: 0 },
        { label: '需要拿拐杖或助行器', value: 'support', score: 2 },
        { label: '只能坐轮椅', value: 'wheelchair', score: 5 }
      ]
    },
    {
      key: 'pain',
      title: '最近腿疼不疼？',
      options: [
        { label: '无疼痛体验', value: 'none', score: 0 },
        { label: '偶发疼痛（通常数周 1 次）', value: 'mild', score: 1 },
        { label: '持续疼痛（每周 3 次以上）', value: 'persistent', score: 5, stop: true },
        { label: '疼痛剧烈（每日发作，甚至无法行走）', value: 'severe', score: 6, stop: true }
      ]
    }
  ],
  actionTests: [
    {
      ...
      options: [
        { label: '双腿垂直，肌肉匀称', value: 'good', score: 0 },
        { label: '有一点不对称', value: 'fair', score: 1 },
        { label: '腿型明显歪斜或很不对称', value: 'poor', score: 2 }
      ]
    },
    {
      key: 'squat',
    ...
    },
    {
      ...
      options: [
        { label: '能很轻松地抬起来', value: 'easy', score: 0 },
        { label: '抬不起来', value: 'fail', score: 2 },
        { label: '抬起来后剧烈抖动', value: 'shake', score: 2 },
    ...
    },
    {
      ...
      options: [
        { label: '超过 10 秒，很稳当', value: 'stable', score: 0 },
    ...
    },
    {
      ...
      options: [
        ...
        { label: '有轻微不稳或酸胀', value: 'fair', score: 1 },
    ...
    },
    {
      ...
      options: [
        ...
        { label: '跳过/不适合做', value: 'skip', score: 1 },
  ...
  riskQuestions: [
    ...
    {
      key: 'dailyImpact',
      title: '您觉得现在的腿脚，影响您过日子吗？',
      options: [
        { label: '完全不影响', value: 'none', score: 0 },
        { label: '有点影响', value: 'some', score: 1 },
...
const knowledgeItems = [
  {
    question: '什么是膝关节炎？',
    answer: '膝关节炎是一种常见的关节问题，常和关节软骨磨损、退化有关，可能出现疼痛、肿胀、僵硬或活动受限。小鹅只能做科普提醒，不能替代医生诊断。'
  },
  {
    question: '膝关节炎的症状有哪些？',
  ...
  {
    question: '膝关节炎患者应该如何锻炼？',
    answer: '适合从低强度开始，例如直腿抬高、踝泵、短时间散步。训练原则是慢、稳、无明显疼痛。'
  },
  {
    question: '膝关节疼痛怎么办？',
    answer: '先暂停运动并观察。轻微酸胀可休息和降低强度；如果持续疼痛、刺痛、肿胀或无法行走，请尽快咨询医生。'
  },
  {
    question: '饮食上有什么建议？',
...
const roleItems = [
  ...
  },
  {
    ...
    desc: '内容、数据和提醒管理入口',
    action: '进入管理台',
    image: '/assets/images/penguin/searching.jpg'
  }
]
...
const loginVoiceTips = [
  {
    keyword: '怎么注册',
    answer: '请先填写姓名，再拖动出生年份，最后点击完成注册就可以进入小鹅陪练。'
  },
  {
    keyword: '年龄怎么选',
    answer: '请拖动出生年份滑条，选到您出生的年份。小鹅会自动帮您记住。'
  },
  {
    keyword: '不会打字',
    answer: '您可以请家人帮忙输入姓名。当前语音助手会先帮您听清问题，再一步步提示怎么填写。'
  }
]
...
const adminDashboard = {
  ...
  tasks: ['训练动作内容维护', '视频号科普内容审核', '高风险测评提醒查看']
}

if (caregiverDashboard.elders && caregiverDashboard.elders[0]) {
  caregiverDashboard.elders[0].training = '今日 2/3'
}

if (caregiverDashboard.elders && caregiverDashboard.elders[1]) {
  caregiverDashboard.elders[1].training = '今日 1/3'
}
